...
struct FavoriteOrderManager {
    ...
    func changeState(orderID: Int, isFavorite: Bool, completion: @escaping(Result<Bool, Error>) -> Void) {
        if isFavorite {
            request(isFavorite: isFavorite, stringUrl: "\(ServerAddressConstants.MAIN_SERVER_ADDRESS)/security/order/favorite/deleteFavorite/\(orderID)", completion: completion)
            return
        }
        request(isFavorite: isFavorite, stringUrl: "\(ServerAddressConstants.MAIN_SERVER_ADDRESS)/security/order/favorite/addFavorite/\(orderID)", completion: completion)
    }
    ...
    private func makeUnfavorite(orderID: Int, completion: @escaping(Result<Bool, Error>) -> Void) {
        guard let url = URL(string: "\(ServerAddressConstants.MAIN_SERVER_ADDRESS)/security/order/favorite/deleteFavorite/\(orderID)") else {
            completion(.failure(NSError()))
            return
        }
    }
