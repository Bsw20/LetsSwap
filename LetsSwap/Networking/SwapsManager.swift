...
struct SwapsManager: SwapsFetcher{
    ...
    func createChat(userId: Int, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let url = URL(string: "http://178.154.210.140:3000/chat/create") else {
            SwiftyBeaver.error(String.incorrectUrl(url: "http://178.154.210.140:3000/chat/create"))
    ...
    func confirmSwap(swapId: Int, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let url = URL(string: "http://178.154.210.140:3000/change/confirm") else {
            SwiftyBeaver.error(String.incorrectUrl(url: "http://178.154.210.140:3000/change/confirm"))
            completion(.failure(NSError()))
            return
        }
        print(swapId)
    ...
    func validateSwap(orderId: Int, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let url = URL(string: "http://178.154.210.140:3000/change/canMakeChange/\(orderId)") else {
            SwiftyBeaver.error(String.incorrectUrl(url: "http://178.154.210.140:3000/change/canMakeChange/\(orderId)"))
            completion(.failure(NSError()))
            return
        }
    ...
    func makeSwap(model: MakeSwapModel, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let url = URL(string: "http://178.154.210.140:3000/change/addChange") else {
            SwiftyBeaver.error(String.incorrectUrl(url: "http://178.154.210.140:3000/change/addChange"))
            completion(.failure(NSError()))
            return
        }
